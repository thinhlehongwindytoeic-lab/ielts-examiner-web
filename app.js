...
// =========================================================
// ============ HỆ THỐNG FLASHCARD (STUDENT/TEACHER) =======
// =========================================================

// Dữ liệu giả lập (Sau này bạn có thể thay bằng API kéo từ Google Sheet)
const fcOriginalList = [
    { word: "Resilience", phonetic: "/rɪˈzɪliəns/", meaning: "Sự kiên cường, khả năng phục hồi" },
    { word: "Meticulous", phonetic: "/məˈtɪkjələs/", meaning: "Tỉ mỉ, cẩn thận" },
    { word: "Obscure", phonetic: "/əbˈskjʊər/", meaning: "Mờ mịt, khó hiểu" },
    { word: "Profound", phonetic: "/prəˈfaʊnd/", meaning: "Sâu sắc, uyên thâm" }
];

let fcCurrentList = [];      // Danh sách đang nạp vào màn hình hiện tại
let fcRevisionList = [];     // Danh sách chứa những từ bị ấn Mũi Tên Xuống
let fcCurrentIndex = 0;      // Con trỏ vị trí
let isFcFlipped = false;     // Trạng thái mặt thẻ

// Gọi các HTML Element ra
const fcCard = document.getElementById('fcCard');
const fcPlayScreen = document.getElementById('fcPlayScreen');
const fcEndScreen = document.getElementById('fcEndScreen');
const flashcardApp = document.getElementById('flashcardApp');

// Hàm bắt đầu học (Dùng để khởi tạo ban đầu)
function initFlashcardMode() {
    // Tạm Ẩn màn hình App cũ, hiện màn hình Flashcard
    document.getElementById('mainApp').style.display = 'none'; 
    document.getElementById('loginScreen').style.display = 'none'; 
    flashcardApp.style.display = 'block';

    fcCurrentList = [...fcOriginalList]; // Nạp 42 từ (ví dụ) vào
    fcRevisionList = [];
    fcCurrentIndex = 0;
    
    document.getElementById('fcTotalLabel').innerText = fcOriginalList.length;
    loadFlashcard();
}

// Logic Nạp dữ liệu của 1 từ lên mặt thẻ
function loadFlashcard() {
    // Nếu lật hết thẻ -> Bật màn hình Kết Thúc
    if (fcCurrentIndex >= fcCurrentList.length) {
        showFcEndScreen();
        return;
    }
    
    // Đảm bảo thẻ luôn nằm ở mặt trước khi nạp từ mới
    isFcFlipped = false;
    fcCard.classList.remove('flipped');
    
    const currentData = fcCurrentList[fcCurrentIndex];
    document.getElementById('fcWord').innerText = currentData.word;
    document.getElementById('fcPhonetic').innerText = currentData.phonetic;
    document.getElementById('fcMeaning').innerText = currentData.meaning;
    
    // Cập nhật bộ đếm (Ví dụ: 1/42)
    document.getElementById('fcProgress').innerText = `${fcCurrentIndex + 1} / ${fcCurrentList.length}`;
    fcPlayScreen.style.display = "block";
    fcEndScreen.style.display = "none";
}

// Logic Màn hình thống kê
function showFcEndScreen() {
    fcPlayScreen.style.display = "none";
    fcEndScreen.style.display = "block";
    
    let total = fcCurrentList.length;
    let fails = fcRevisionList.length;
    
    document.getElementById('fcScoreRevise').innerText = fails;
    document.getElementById('fcScorePass').innerText = total - fails;
    
    // Nếu có lỗi sai -> Hiện nút Ôn Từ Chưa Đạt
    if (fails > 0) {
        document.getElementById('btnReviseMistakes').style.display = "block";
    } else {
        document.getElementById('btnReviseMistakes').style.display = "none";
    }
}

// Logic lật thẻ & Phát âm thanh mặc định của trình duyệt
function toggleFlashcard() {
    ...
    if (isFcFlipped) {
        ...
        // Phát âm thanh
        const utterance = new SpeechSynthesisUtterance(fcCurrentList[fcCurrentIndex].word);
...
document.addEventListener('keydown', (e) => {
    // Chỉ kích hoạt phím khi Flashcard đang hiện
    if (flashcardApp.style.display === "none" || fcPlayScreen.style.display === "none") return;
    ...
    switch(e.code) {
        ...
        case 'ArrowRight': // ĐẠT (Passed)
            e.preventDefault();
            fcCurrentIndex++;
            loadFlashcard();
            break;
            
        case 'ArrowDown': // CẦN ÔN LẠI (Revision)
            e.preventDefault();
            // Đưa từ này vào danh sách sai (chống đưa trùng lặp)
            const currentWord = fcCurrentList[fcCurrentIndex];
            if (!fcRevisionList.some(item => item.word === currentWord.word)) {
                fcRevisionList.push(currentWord);
            }
            fcCurrentIndex++;
            loadFlashcard();
            break;
            
        case 'ArrowLeft': // UNDO (Lùi lại)
            e.preventDefault();
            if (fcCurrentIndex > 0) {
                fcCurrentIndex--;
                // Nếu từ này lỡ bị ấn Mũi Tên Xuống, thì khi Undo phải gỡ nó ra khỏi mảng Revision
                fcRevisionList = fcRevisionList.filter(item => item.word !== fcCurrentList[fcCurrentIndex].word);
                loadFlashcard();
            }
            break;
            
        case 'ArrowUp': // SKIP (Bỏ qua luôn - Dành cho Giáo viên)
            e.preventDefault();
            fcCurrentIndex++;
            loadFlashcard();
            break;
...
// Nút CLICK: Reset Session và chỉ load TỪ SAI
document.getElementById('btnReviseMistakes').addEventListener('click', () => {
    fcCurrentList = [...fcRevisionList]; // Đè danh sách hiện tại bằng danh sách bị sai
    fcRevisionList = []; // Reset lại giỏ chứa lỗi sai
    fcCurrentIndex = 0;
    loadFlashcard();
});

// Nút CLICK: Reset Session và load TOÀN BỘ GỐC
document.getElementById('btnReviseAll').addEventListener('click', () => {
    fcCurrentList = [...fcOriginalList]; // Đè bằng danh sách 42 từ gốc
    fcRevisionList = []; 
    fcCurrentIndex = 0;
    loadFlashcard();
});

// --- CODE TEST (Tạm thời tự động bật giao diện Flashcard khi vừa load trang) ---
// Bạn có thể xóa dòng này nếu muốn ghép nút bấm bên ngoài để mở
setTimeout(initFlashcardMode, 500);
